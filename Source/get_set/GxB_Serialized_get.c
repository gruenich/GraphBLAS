...
// now in GrB v10.0.0:
    ...
    (*p_control) = GB_RSHIFT (control_encoding, 16, 4) ;
    (*j_control) = GB_RSHIFT (control_encoding, 12, 4) ;
    (*i_control) = GB_RSHIFT (control_encoding,  8, 4) ;
...
)
{
    ...
    info = GB_blob_header_get (type_name, &type_code,
