...
// operator:
#define GB_BINOP(z,x,y,i,j) z = GB_FC32_iseq (x, y)
...
#if (defined(GxB_NO_ISEQ) || defined(GxB_NO_FC32) || defined(GxB_NO_ISEQ_FC32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
GrB_Info GB (_Cewise_fulln__iseq_fc32)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_AunionB__iseq_fc32)
(
    GrB_Matrix C,
    const int C_sparsity,
    const GrB_Matrix M,
