...
// operator:
#define GB_BINOP(z,x,y,i,j) z = GB_FC32_isne (x, y)
...
#if (defined(GxB_NO_ISNE) || defined(GxB_NO_FC32) || defined(GxB_NO_ISNE_FC32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
GrB_Info GB (_Cewise_fulln__isne_fc32)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_AunionB__isne_fc32)
(
    GrB_Matrix C,
    const int C_sparsity,
    const GrB_Matrix M,
