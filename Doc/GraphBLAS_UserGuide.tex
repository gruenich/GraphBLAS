...
new _Vector methods for build, extract, extractTuples, assign, subassign:
    ...
    The Container->[phbix] GrB_Vectors should not be freed by the user
    ...
    is freed.  If they are somehow freed, simply free the entire Container and
    recreate it, or replace the missing vectors with length-0 vectors of any
    type; for example:

        GrB_Vector_new (&(Container->p), GrB_UINT32, 0) ;

GrB_Field enum deprecated
...
\item XX, 2025: version 10.0.0
    ...
    \item AppleClang compiler bug:  On the Mac, the
...
\item Dec 20, 2024: version 9.4.3
    ...
    \item (57) bug fix: GraphBLAS.h header: remove duplicate definitions of
        \verb'GxB_MAX_FIRST_*' semirings (incompletely moved to 'historical'
        section in 9.4.2).
    \end{itemize}
